    ...
    public static void main(String[] args) {
        ...
        try (final Connection connection = connectionFactory.newConnection();
            ...
            DeliverCallback deliverCallback = ((consumerTag, delivery) -> {
                ...
                Feedback feedback = new GsonBuilder().create().fromJson(feedbackMessage, Feedback.class);

                CommentTone comment = new CommentTone(feedback.id, feedback.message);

                System.out.println("feedbacks : " + feedbackMessage);

                CommentTonePublisher publisher = new CommentTonePublisher(publishExchangeName, serviceName);
